<script lang="ts">
  ...
  export let manifestUrl: string = '/terrain/{levelId}.manifest.json' // Generated manifest path
  ...
  async function loadLevelFromManifest() {
    try {
      ...
      terrainConfig = {
        ...
        // TriMesh physics configuration from manifest
        useTrimesh: manifest.physics.type === 'trimesh',
        trimeshMode: manifest.physics.trimesh?.mode || 'single',
        trimeshDownsample: manifest.physics.trimesh?.downsample || 16,
        trimeshChunkVerts: manifest.physics.trimesh?.chunkVerts || 65,
        trimeshActiveRadius: manifest.physics.trimesh?.activeRadius || 2
      };
...
{#if manifest}
  <LevelManager let:registry let:lighting let:ecsWorld>
    ...
    <!-- Dynamic fog: use ocean underwater settings when submerged, style fog otherwise -->
    <T.FogExp2 
      color={$underwaterStateStore.isUnderwater
        ? (manifest.ocean?.underwaterFogColor ?? manifest.style?.fog?.color ?? '#006994')
        : (manifest.style?.fog?.color ?? '#6a7db3')} 
      density={$underwaterStateStore.isUnderwater
        ? (manifest.ocean?.underwaterFogDensity ?? (manifest.style?.fog?.density ? manifest.style.fog.density * 50 : 0.1))
    ...
    <T.Group name={manifest.id}>
      ...
      {#if manifest.features.ocean}
        <OceanComponent 
          ...
          waterCollisionSize={[
            manifest.ocean?.size?.width || 1000, 
            20, 
            manifest.ocean?.size?.height || 1000
          ]}
