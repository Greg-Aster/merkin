...
if [[ -d "$EXPORT_DIR/.git" ]]; then
  echo "    Reusing existing export directory and .git metadata"
else
  echo "    Created export directory (no .git found — run git init manually)"
fi

mkdir -p "$EXPORT_DIR/packages"
mkdir -p "$EXPORT_DIR/apps"
mkdir -p "$EXPORT_DIR/scripts"
rm -rf "$EXPORT_DIR/.github/workflows"
mkdir -p "$EXPORT_DIR/.github/workflows"

# Remove files that used to be generated by older exports but should not persist.
rm -f \
  "$EXPORT_DIR/scripts/generate-post-pdfs.mjs" \
  "$EXPORT_DIR/scripts/sync-to-temporal-flow.sh"
...
# -----------------------------------------------------------------------------
# 3. Copy Temporal-Flow as apps/Temporal-Flow (copy everything; it IS the template)
# -----------------------------------------------------------------------------
echo "==> [3/7] Creating apps/Temporal-Flow from Temporal-Flow..."
...
# Strip Merkin-specific build step from package.json (keep original name "temporal-flow")
...
# Users with a custom domain should create apps/Temporal-Flow/CNAME containing their domain.
rm -f "$EXAMPLE_DST/CNAME"

# Fix Tailwind content path: the app is nested under apps/ so blog-core is two levels up.
# In Merkin: Temporal-Flow/ → ../packages/blog-core (one level up)
# In export: apps/Temporal-Flow/ → ../../packages/blog-core (two levels up)
...
echo "    Done — apps/Temporal-Flow (full Temporal-Flow copy)"
