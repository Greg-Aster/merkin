...
import LevelEditorBottomDock from "./LevelEditorBottomDock.svelte";
...
import {
	type LevelEditorWorkbenchBottomDockRegion,
	buildLevelEditorWorkbenchModel,
} from "./levelEditorWorkbenchModel.js";
import type {
...
let selectedBottomDockTabId: BottomDockTabId = $state("content-browser");
const dirtyCount = $derived(authoringQueue.dirtyCount);
const hasDirtyState = $derived(dirtyCount > 0);
const bottomDockTabs = $derived(
	workbench.regions.bottomDock.tabs.map((tab) => ({
		...tab,
		itemCount:
...
function selectBottomDockTab(tabId: BottomDockTabId): void {
	selectedBottomDockTabId = tabId;
...
<LevelEditorBottomDock
	...
	{editorSession}
	tabs={bottomDockTabs}
	selectedTabId={selectedBottomDockTabId}
	{selectedCommandPlanId}
	{status}
	{objectLibraryPanelModel}
	selectedObjectLibraryEntryId={objectLibraryPanelModel.selectedEntryId}
	...
	{runtimeTelemetry}
	{runtimeTelemetryState}
	runtimeLifecycleLabel={runtimeLifecycleLabel()}
	runtimePositionLabel={formatRuntimePosition()}
	runtimeHealthLabel={formatRuntimeHealth()}
	runtimeInputLabel={formatRuntimeInput()}
	runtimeChargeLabel={formatRuntimeCharge()}
	onSelectTab={selectBottomDockTab}
	onSelectCommandPlan={(planId) => (selectedCommandPlanId = planId)}
	onSelectObjectLibraryEntry={selectObjectLibraryEntry}
	onStageObjectLibraryReplacement={stageObjectLibraryReplacement}
	onStageAuthoringOperations={stageAuthoringOperationEntry}
	onRemoveQueuedAuthoringOperationEntry={removeQueuedAuthoringOperationEntry}
	onRemoveStagedFieldEdit={removeStagedFieldEdit}
