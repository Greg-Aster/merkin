...
const { mounted, snapshot, gameState, startupError, dispatch }: Props =
	$props();

const formatPosition = (position: readonly [number, number, number]) =>
	position.map((value) => value.toFixed(1)).join(" / ");

const inputStatus = () => {
	if (gameState.lookActive) {
		return "looking";
	}
	...
	return gameState.inputEnabled ? "ready" : "paused";
};
...
<section class="hud" aria-label="Game status" data-engine-status>
	<div class="hud-row hud-topline">
		...
		<span class:error={startupError}>
			{startupError ?? (mounted ? snapshot.lifecycle : "loading")}
		</span>
	</div>
	<div class="hud-grid" aria-live="polite">
		<div>
			<span>Health</span>
			<strong>{gameState.health[0]} / {gameState.health[1]}</strong>
		</div>
		<div>
			<span>Collected</span>
			<strong>{gameState.collectedCount}</strong>
		</div>
		<div>
			<span>Remaining</span>
			<strong>{gameState.remainingCollectibles}</strong>
		</div>
		<div>
			<span>Position</span>
			<strong>{formatPosition(gameState.playerPosition)}</strong>
		</div>
		<div>
			<span>Tick</span>
		...
		<div>
			<span>Move</span>
			<strong>{gameState.moving ? "active" : "idle"}</strong>
		</div>
		<div>
			<span>Input</span>
		...
		<div>
			...
			<strong>
				{gameState.charging
					? `${Math.round(gameState.chargeAmount * 100)}%`
					: "idle"}
			</strong>
		</div>
	</div>
...
{#if !gameState.openStoryNote && gameState.activePortal}
	<div
...
{:else if !gameState.openStoryNote && gameState.activeStoryNote}
