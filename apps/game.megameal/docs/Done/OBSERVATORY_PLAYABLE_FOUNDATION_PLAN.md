...
terrain, temporary explicit collision proxy, spawn, sky, shared static water surface,
...
- Add Observatory V1 authored proxy collision as a temporary level consumer of
  ...
  - render asset IDs require matching authored or cooked collision products;
    the missing Observatory environment collision product remains a tracked
    quality gap
...
- Content/manifests/assets were implemented:
  - `src/levels/observatory/assets.ts`
...
- Observatory V1 collision content implemented as a temporary proxy layer over
  the engine-wide collision contracts:
...
- Cooked static environment collision for `mesh_observatory_environment`,
