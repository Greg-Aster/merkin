...
| --- | --- | --- |
| Miranda migration | Continue extracting old Miranda content into checked-in target-engine level, prefab, readiness, portal, audio, and collision data. | `src/levels/miranda-deck/`, shared package owners in `src/levels/global/` when the packet specifically requires them, and migration docs. |
| Observatory playable foundation | Build the target-engine Observatory foundation with visual level data, authored collision proxies, shared water/environment use, light readiness, and portal reachability. | `src/levels/observatory/`, shared package owners in `src/levels/global/` when intentionally shared, Observatory-specific docs, and reusable environmental contracts. |
...
| `src/levels/<level>/manifest.ts` | Runtime scenes can list level-owned requirements, but those requirements must stay inside the owning level folder. |
| `src/levels/global/router.ts` | The router is the only checked-in package catalog/default-selection owner. |
| `src/game/prefabs/index.ts` | Only generic registry/spawn mechanics belong here. Do not add product prefab data, asset catalogs, dead exports, or temporary staging modules. |
