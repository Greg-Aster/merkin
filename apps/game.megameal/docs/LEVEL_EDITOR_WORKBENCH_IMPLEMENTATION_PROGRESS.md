...
- Current strengths: dev-only boundary, runtime catalog selection, stable-ID
  objects, grouped outliner data, inspector fields, dev-preview bridge,
...
- Current major gaps: central viewport/direct picking, rendered-scene transform gizmos,
  ...
  normalized viewport drop placement, owner writes for component
  families that need non-level owners, and full
  professional editor ergonomics beyond the first workbench shell.
- Known validation status: the latest recorded full focused editor gate set,
  aggregate contract suite, type-check, lint, boundary audit, build, and diff
  check passed after enabling generated owner writes for bounded
  `remove-level-instance` records, including generated insertion cancellation
  and stale generated record clearing. Build still reports the existing
...
| --- | --- | --- |
| Workbench shell | First professional shell integrated | Needs runtime-backed viewport depth and final layout polish |
| Selection | Stable-ID selection drives outliner, inspector, viewport pins, and object-library context | Needs true live viewport picking against rendered scene objects |
| Inspector | Editable fields stage authoring edits with workflow labels, including level-instance `set-component` save operations for supported component fields | Needs broader component editors and owner-write families beyond level-instance component set/removal records |
| Save/Publish | Save Draft plus bounded generated-owner Save Level/Publish for transforms, object-library placements, level-instance prefab ID replacements, level-instance component overrides/removals, bounded level-instance duplicate insertions, and bounded level-instance removals are guarded | Needs publishable coverage for collision, terrain, audio manifest, NPCs, environment/render-profile edits, asset-manifest edits, prefab-owned component edits, readiness-owner deletes/replacements, broad asset/component replacement, grouping, stable-ID management, and undo/redo publish semantics |
| Viewport bridge | Projected pins, transform nudges, snap controls, screen-space transform handles, placement target, placement ghost, and normalized object-library drop placement exist | Needs real gizmos, arbitrary drag placement with rendered-scene hit testing, richer camera/navigation controls, and rendered-scene hit testing |
| Content browser | Search/filter, previews, publish-ready placement readiness, transform composer, staged review/removal, viewport ghost handoff, and draft-ready drag metadata exist | Needs asset import workflows, richer placement ergonomics, and full rendered viewport placement |
...
- Added screen-space transform handles anchored to the selected projected
  ...
  they still do not claim real rendered-scene picking, drag handles, or runtime
  mutation.
...
- Direct manipulation remains disabled and explicitly modeled as future work.
...
- Placement is publishable only for generated object-library insertion
  operations. The current drop target is a bounded normalized top-down
  placement surface derived from authored transforms; rendered-scene drop
  projection, arbitrary surface hit testing, and authored transform picking
  remain future work.
