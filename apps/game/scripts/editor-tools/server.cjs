  ...
  if (pathname === '/api/editor-scene/cook-world-partition' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        child.on('error', error => {
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ success: false, message: `World partition cook process error: ${error.message}` }));
        });
      } catch (error) {
        console.error('Editor world partition cook error:', error);
        res.writeHead(500, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ success: false, message: 'World partition cook failed: ' + error.message }));
      }
    });
    return;
  }
