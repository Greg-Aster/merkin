...
const args = normalizeSmokeArgs(rawArgs)
...
const noServer =
  ...
  process.env.GAME_NO_SERVER === '1' ||
  isTruthyConfig(process.env.npm_config_no_server)
...
function normalizeSmokeArgs(argv) {
  ...
  if (
    !normalized.includes('--no-server') &&
    (isTruthyConfig(process.env.npm_config_no_server) ||
      isTruthyConfig(process.env.npm_config_noServer))
  ) {
    normalized.push('--no-server')
  }
...
function isTruthyConfig(value) {
  ...
  return !['', '0', 'false', 'undefined', 'null'].includes(
    String(value).toLowerCase(),
  )
}

async function runEditorUxSmoke() {
  ...
  try {
    ...
    if (exitCode !== 0) {
      throw new Error(`Editor UX smoke check failed with exit code ${exitCode}`)
    }
  } finally {
