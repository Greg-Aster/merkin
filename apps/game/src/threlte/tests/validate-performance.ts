...
export class PerformanceValidator {
  ...
  private calculateMetrics(testName: string): PerformanceMetrics {
    ...
    const fpsSamples = this.samples
      .map(s => s.fps)
      .filter(fps => fps > 0 && fps < 1000)
    const frameTimeSamples = this.samples
      .map(s => s.frameTime)
      .filter(ft => ft > 0 && ft < 1000)
    ...
    return {
      ...
      memory: {
        ...
        delta:
          (memorySamples[memorySamples.length - 1] || 0) -
          (memorySamples[0] || 0),
      },
  ...
  public validatePerformance(
    ...
    if (metrics.fps.average < defaultTargets.minFPS) {
      issues.push(
        `Average FPS (${metrics.fps.average.toFixed(1)}) below target (${defaultTargets.minFPS})`,
      )
      score -= 30
    }
    ...
    if (metrics.frameTime.p95 > defaultTargets.maxFrameTime) {
      issues.push(
        `95th percentile frame time (${metrics.frameTime.p95.toFixed(2)}ms) exceeds target`,
      )
      score -= 15
    }
    ...
    if (metrics.memory.delta > defaultTargets.maxMemoryDelta) {
      issues.push(
        `Memory increase (${(metrics.memory.delta / 1024 / 1024).toFixed(1)}MB) exceeds target`,
      )
      score -= 10
    }
    ...
    if (metrics.render.drawCalls > defaultTargets.maxDrawCalls) {
      issues.push(
        `Draw calls (${metrics.render.drawCalls}) exceed target (${defaultTargets.maxDrawCalls})`,
      )
      score -= 10
    }
    ...
    if (frameTimeVariance > 50) {
      issues.push(
        `High frame time variance (${frameTimeVariance.toFixed(2)}ms)`,
      )
      score -= 5
    }
  ...
  private generateRecommendations(
    ...
    if (metrics.render.triangles > 500000) {
      recommendations.push(
        'Use mesh simplification or LOD for high polygon models',
      )
    }
    ...
    if (issues.length === 0 && metrics.fps.average > 55) {
      recommendations.push(
        'Performance is excellent! Consider enabling higher quality settings',
      )
    }
  ...
  public comparePerformance(
    ...
    const comparison = {
      ...
      improvement: {
        fps:
          ((current.fps.average - baseline.fps.average) /
            baseline.fps.average) *
          100,
        frameTime:
          ((baseline.frameTime.average - current.frameTime.average) /
            baseline.frameTime.average) *
          100,
        memory:
          ((baseline.memory.delta - current.memory.delta) /
            Math.max(baseline.memory.delta, 1)) *
          100,
      },
    }
    ...
    if (comparison.improvement.fps > 10) {
      ...
      result.recommendations.unshift(
        'FPS regression detected - investigate recent changes',
      )
    }
...
export const PerformanceTestUtils = {
  ...
  async compareVersions(
    ...
    const comparisonResult = validator.comparePerformance(
      threeJSMetrics,
      threlteMetrics,
    )
