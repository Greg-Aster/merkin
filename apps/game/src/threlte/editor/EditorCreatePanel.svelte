...
export let assetBrowserItems: Array<{
  name: string
  path: string
  isDirectory: boolean
}> = []
...
export let selectedLibraryItem: {
  name: string
  path: string
  isDirectory: boolean
} | null = null
...
export let onSelectLibraryItem: (item: {
  name: string
  path: string
  isDirectory: boolean
}) => void = () => {}
...
<div class="editor-section">
  ...
  <div class="tuple-group">
    ...
    <textarea rows="4" bind:value={hunyuanScratchPrompt} data-sfx-focus="focus-soft" placeholder="Describe the new mesh you want in the library."></textarea>
  </div>
  <div class="button-row compact-two-columns">
    <button data-sfx-hover="hover-emphasis" data-sfx-click="confirm" disabled={hunyuanBusy || !hunyuanBackendCanGenerate} on:click={onGenerateToLibrary}>{hunyuanBusy ? 'Working…' : 'Generate To Library'}</button>
    <button data-sfx-hover="hover-emphasis" data-sfx-click="confirm" disabled={hunyuanBusy || !hunyuanBackendCanGenerate} on:click={onGenerateAndAdd}>{hunyuanBusy ? 'Working…' : 'Generate + Add'}</button>
  </div>
...
<div class="editor-section">
  ...
  <div class="button-row compact-two-columns">
    <button class:active={assetBrowserPath.startsWith(modelsRootPath)} data-sfx-hover="hover-soft" data-sfx-click="select" on:click={() => onSelectAssetLibraryRoot(modelsRootPath)}>Imported Models</button>
    <button class:active={assetBrowserPath.startsWith(generatedRootPath)} data-sfx-hover="hover-soft" data-sfx-click="select" on:click={() => onSelectAssetLibraryRoot(generatedRootPath)}>Generated Assets</button>
  </div>
  <div class="button-row compact editor-mt-sm">
  ...
  <div class="tuple-group editor-mb-sm">
    ...
    <input class="text-input" bind:value={assetBrowserFilter} data-sfx-focus="focus-soft" placeholder="Filter asset names" />
  </div>
  ...
  {#if selectedLibraryItem && !selectedLibraryItem.isDirectory}
    <div class="editor-subsection">
      ...
      <div class="button-row compact-two-columns editor-mt-sm">
        ...
        <button data-sfx-hover="hover-soft" data-sfx-click="panel-open" disabled={hunyuanBusy || !selectedLibraryItemUrl} on:click={onInspectSelectedLibraryAsset}>Inspect AI</button>
      </div>
      {#if assetPickerTargetNodeId}
        <div class="button-row compact editor-mt-sm">
      ...
      <div class="tuple-group">
        ...
        <textarea rows="3" bind:value={hunyuanPrompt} data-sfx-focus="focus-soft" placeholder="Describe how to reimagine this asset."></textarea>
      </div>
      <div class="tuple-group">
        <div class="tuple-label">Reference Image</div>
        <input class="text-input" bind:value={hunyuanReferenceImageUrl} data-sfx-focus="focus-soft" placeholder="Optional override reference image" />
      </div>
      ...
      <div class="button-grid editor-mt-sm">
        <button data-sfx-hover="hover-emphasis" data-sfx-click="confirm" disabled={hunyuanBusy || !hunyuanBackendCanGenerate} on:click={onRunLibraryGenerate}>
          {hunyuanBusy ? 'Working…' : 'Reimagine To New Asset'}
        </button>
        <button data-sfx-hover="hover-emphasis" data-sfx-click="confirm" disabled={hunyuanBusy || !hunyuanBackendCanRetexture} on:click={onRunLibraryTexture}>
