...
interface EditorAiControllerDeps {
  ...
  getSelectedLibraryItem: () => { name: string, path: string, isDirectory: boolean } | null
  ...
  getSceneNodeVisualBounds: (node: EditorSceneNode, sourceAssetUrl?: string) => Promise<{ size: [number, number, number] }>
  fitGeneratedAssetToSource: (nodeId: string, sourceVisualBounds: { size: [number, number, number] }, assetUrl: string, baseScale: [number, number, number]) => Promise<{ appliedScale: [number, number, number], report: string }>
  ...
  setRuntimeDiagnostic: (source: string, payload: { level: 'ready' | 'warning' | 'error' | 'loading', message: string }) => void
...
export function createEditorAiController(deps: EditorAiControllerDeps) {
  ...
  async function refreshHunyuanServiceStatus(ensure = false) {
    try {
      ...
      if (!payload?.success || !payload?.status) {
        ...
        state.hunyuanBackendStatus = payload?.message ?? 'Mesh backend unavailable.'
      ...
      state.hunyuanBackendCanGenerate = !!payload.status.supportsReplacementGeneration
      state.hunyuanBackendCanRetexture = !!payload.status.supportsTextureWrap
      state.hunyuanBackendStatus = payload.status.message ?? state.hunyuanBackendStatus
      deps.setRuntimeDiagnostic('hunyuan', {
  ...
  async function openComfyUiWorkflowEditor(mode: 'generate' | 'texture') {
    ...
    const sourceName = sourceNode ? deps.getAiSourceName(sourceNode) : (state.hunyuanScratchName.trim() || 'Generated Asset')
    ...
    try {
      deps.appendPipelineLog('Opening ComfyUI workflow editor', { mode, workflowPath: state.selectedComfyWorkflowPath, sourceName, assetUrl })
      const response = await fetch(
      ...
      const payload = await deps.readJsonPayload(response, 'Workflow template request')
      ...
      if (typeof window !== 'undefined') {
        window.open(payload.editorUrl || state.comfyUiApiUrl, '_blank', 'noopener,noreferrer')
      }

      state.comfyWorkflowEditorStatus = payload.message ?? 'Workflow copied to clipboard and ComfyUI opened.'
      state.saveMessage = state.comfyWorkflowEditorStatus
    } catch (error) {
      console.error('Open ComfyUI workflow editor failed:', error)
      state.comfyWorkflowEditorStatus = error instanceof Error
        ? error.message
  ...
  async function refreshHunyuanRecentJobs(limit = 10) {
    ...
    try {
      ...
      if (!payload?.success || !Array.isArray(payload.jobs)) {
        ...
        state.hunyuanJobsError = payload?.message ?? 'Could not load recent Hunyuan jobs.'
        return
      }

      state.recentHunyuanJobs = payload.jobs
      if (!state.selectedHunyuanJobId || !state.recentHunyuanJobs.some((job: any) => job.id === state.selectedHunyuanJobId)) {
        state.selectedHunyuanJobId = state.recentHunyuanJobs.find((job: any) => job.status === 'failed')?.id
  ...
  async function queueHunyuanJob(requestBody: Record<string, unknown>) {
    ...
    const queuePayload = await deps.readJsonPayload(queueResponse, 'Hunyuan job queue')

    if (!queuePayload?.success || !queuePayload?.job?.id) {
      throw new Error(queuePayload?.message ?? 'Could not queue the Hunyuan job.')
    }

    deps.appendPipelineLog('Queued Hunyuan job', { jobId: queuePayload.job.id, queuePosition: queuePayload.job.queuePosition ?? null })
  ...
  async function getHunyuanJobStatus(jobId: string) {
    ...
    const statusPayload = await deps.readJsonPayload(statusResponse, 'Hunyuan job status')
  ...
  async function waitForQueuedHunyuanJob(
    ...
    while (true) {
      ...
      if (job.status === 'queued') {
        ...
        state.hunyuanStatus = queuePosition > 1
      ...
      if (job.status === 'running') {
        ...
        state.hunyuanStatus = 'Generating asset with ComfyUI + Hunyuan… this can take a while.'
        deps.setRuntimeDiagnostic('hunyuan', {
          level: 'loading',
          message: state.hunyuanStatus,
        })
      ...
      if (job.status === 'failed') {
        ...
        state.hunyuanBackendCanGenerate = !!result?.status?.supportsReplacementGeneration
  ...
  async function queueAndWaitForHunyuanJob(requestBody: Record<string, unknown>) {
    ...
    state.hunyuanStatus = initialQueuePosition > 1
  ...
  async function runHunyuanForSelection(mode: 'generate' | 'texture') {
    ...
    if (!selectedNode || !deps.canUseAiMeshStudio(selectedNode)) {
      state.hunyuanStatus = 'Select a single geometry node before running Hunyuan.'
      return
    }
    ...
    const targetNodeFitData = await Promise.all(targetNodeIds.map(async (nodeId) => {
      const node = editorNodes.find((candidate) => candidate.id === nodeId)
      if (!node) return null
      return {
        nodeId,
        sourceVisualBounds: await deps.getSceneNodeVisualBounds(node, targetAssetUrl),
    ...
    if (mode === 'texture' && !targetAssetUrl) {
      state.hunyuanStatus = 'Texture wrapping needs an imported mesh asset. Generate a replacement mesh first for prefabs.'
      return
    }
    ...
    try {
      ...
      const fitDataById = new Map(targetNodeFitData.filter(Boolean).map((entry: any) => [entry.nodeId, entry]))
      const replacementPatches = await Promise.all(targetNodeIds.map(async (nodeId) => {
        const fitData = fitDataById.get(nodeId)
        const fitResult = fitData
          ? await deps.fitGeneratedAssetToSource(nodeId, fitData.sourceVisualBounds, payload.assetUrl, fitData.baseScale)
          : null
        if (fitResult?.report) {
        ...
        return {
          ...
          patch: {
            ...
            generation: {
              ...(editorNodes.find((candidate) => candidate.id === nodeId)?.generation ?? {}),
              descriptor: fitData?.descriptor ?? deps.getDefaultStyleDescriptor(selectedNode),
      ...
      deps.appendPipelineLog('Replaced node(s) with generated asset using preserved transform', {
      ...
      state.hunyuanLastResultSummary = targetNodeIds.length > 1
      ...
      state.saveMessage = targetNodeIds.length > 1
      ...
      state.hunyuanStatus = error instanceof Error
        ? error.message
  ...
  async function runHunyuanToLibrary(options?: { addToScene?: boolean }) {
    ...
    if (!prompt && !referenceImageUrl) {
      state.hunyuanStatus = 'Enter a prompt or reference image before generating a new asset.'
      return
    }
    if (!state.hunyuanBackendCanGenerate) {
      await refreshHunyuanServiceStatus(true)
    ...
    try {
      ...
      state.hunyuanServiceReady = true
      state.hunyuanStatus = payload.message ?? `Generated ${sourceName} into the asset library.`
      deps.setRuntimeDiagnostic('hunyuan', {
        level: 'ready',
        message: state.hunyuanStatus,
      })
  ...
  async function runHunyuanForLibraryAsset(mode: 'generate' | 'texture') {
    ...
    if (!selectedLibraryItem || selectedLibraryItem.isDirectory) {
      state.hunyuanStatus = 'Select a library asset before using AI reimagine tools.'
      return
    }
    ...
    if (mode === 'texture') {
      ...
      if (!state.hunyuanSupportsTextureWrap) {
        await deps.inspectSelectedAssetForHunyuan(assetUrl, selectedLibraryItem.path)
        if (!state.hunyuanSupportsTextureWrap) {
          state.hunyuanStatus = 'Texture wrapping currently supports only compatible mesh assets.'
          return
        }
      }
    } else {
    ...
    state.hunyuanBusy = true
    state.hunyuanStatus = mode === 'texture'
    ...
    try {
      ...
      state.hunyuanServiceReady = true
      state.hunyuanStatus = payload.message ?? `${sourceName} generated into the asset library.`
      deps.setRuntimeDiagnostic('hunyuan', {
        level: 'ready',
        message: state.hunyuanStatus,
      })
