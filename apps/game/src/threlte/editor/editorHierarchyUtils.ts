...
export interface EditorNodeLocalTransform {
  position: [number, number, number]
  rotation: [number, number, number]
  scale: [number, number, number]
}

export function collectDescendantIds(nodes: EditorSceneNode[], nodeId: string) {
...
export function getWorldMatrixForNode(
...
function decomposeMatrix(matrix: THREE.Matrix4): EditorNodeLocalTransform {
...
export function createWorldMatrixResolver(nodes: EditorSceneNode[]) {
  const lookup = createNodeLookup(nodes)
