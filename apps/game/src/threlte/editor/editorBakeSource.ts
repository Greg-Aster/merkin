...
const PREFAB_ASSET_URLS: Partial<Record<EditorPrefabType, string>> = {
  'hanging-light':
    '/models/polyhaven/caged_hanging_light/caged_hanging_light_1k.gltf',
}
...
async function buildSceneNodeSubtreeObject(
  ...
  if (node.primitive) {
    ...
    const prefabObject = prefabAssetUrl
      ...
      : buildProceduralPrefabObject(node)
    if (prefabObject) {
