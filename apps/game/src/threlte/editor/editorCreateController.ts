...
export function createEditorCreateController(deps: EditorCreateControllerDeps) {
  ...
  function addFireflyToSelection() {
    ...
    if (targetNodes.length === 0) {
      deps.setSaveMessage('Select one or more scene objects before adding fireflies')
      return
    }
    ...
    if (createdIds.length > 0) {
      ...
      deps.setSaveMessage(`Added ${createdIds.length} firefl${createdIds.length === 1 ? 'y' : 'ies'} to selection`)
    }
  }

  function addPrefabWithParent(name: string, type: EditorPrefabType, position: [number, number, number] = [0, 0, 0]) {
    deps.editorPrefabs.addPrefab(name, type, position, deps.getSelectedNode()?.id ?? null)
  }

  function addAssetPrefab(name: string, url: string, scale: [number, number, number] = [0.001, 0.001, 0.001]) {
    deps.editorPrefabs.addAsset(name, url, deps.getSelectedNode()?.id ?? null, scale)
  }
