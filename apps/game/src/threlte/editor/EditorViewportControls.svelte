<script lang="ts">
  ...
  const initialObjectMatrices = new Map<string, { worldMatrix: THREE.Matrix4; parentInverse: THREE.Matrix4 }>()
  ...
  const unsubscribe = editorStateStore.subscribe((state) => {
    ...
    if (transformControls) {
      ...
      transformControls.setRotationSnap(snappingEnabled ? THREE.MathUtils.degToRad(rotateSnap) : null)
  ...
  const unsubscribeViewportState = editorNodeViewportStateStore.subscribe((value) => {
  ...
  function syncSelectionAttachment() {
    ...
    if (selectedObjects.length === 1 && transformableSelectedNodeIds.length === 1) {
      multiSelectionActive = false
      transformControls.attach(selectedObjects[0])
    } else if (selectedObjects.length > 1) {
      multiSelectionActive = true
      const center = new THREE.Vector3()
      selectedObjects.forEach((object) => center.add(object.getWorldPosition(new THREE.Vector3())))
  ...
  function updateSelectedNodeTransform(commitNodeUpdate = true) {
    ...
    if (multiSelectionActive) {
      const deltaMatrix = new THREE.Matrix4().multiplyMatrices(selectionPivot.matrixWorld, new THREE.Matrix4().copy(initialPivotMatrix).invert())
      for (const nodeId of transformableSelectedNodeIds) {
        ...
        const nextWorldMatrix = new THREE.Matrix4().multiplyMatrices(deltaMatrix, initial.worldMatrix)
        const nextLocalMatrix = new THREE.Matrix4().multiplyMatrices(initial.parentInverse, nextWorldMatrix)
    ...
    const activeNodeId = selectedNodeId && isNodeSelectable(selectedNodeId) ? selectedNodeId : null
  ...
  function snapObjectToSurface(object: THREE.Object3D, ignoredNodeIds: Set<string>) {
    if (!scene) return
    ...
    const targets = scene.children.filter((child) => child !== transformControls && child !== selectionPivot)
  ...
  function getCircleSelectedNodeIds(clientX: number, clientY: number, radius: number) {
  ...
  function applyCircleSelection(clientX: number, clientY: number, subtracting = false) {
    const hitNodeIds = getCircleSelectedNodeIds(clientX, clientY, circleSelectRadius)
  ...
  function activateCircleSelectTool() {
    ...
    const fallbackX = renderer ? renderer.domElement.getBoundingClientRect().left + (renderer.domElement.getBoundingClientRect().width / 2) : 0
    const fallbackY = renderer ? renderer.domElement.getBoundingClientRect().top + (renderer.domElement.getBoundingClientRect().height / 2) : 0
  ...
  function finalizeMarqueeSelection(clientX: number, clientY: number) {
    ...
    const dragDistance = Math.hypot(clientX - marqueeStart.x, clientY - marqueeStart.y)
  ...
  function handleWindowPointerMove(event: PointerEvent) {
    ...
    if (circleSelectActive) {
      ...
      if (circleSelectSelecting) {
        applyCircleSelection(event.clientX, event.clientY, circleSelectSubtracting)
      }
    }
  ...
  function startMarqueeSelection(event: PointerEvent, options?: { fromBoxTool?: boolean }) {
  ...
  function handleCanvasPointerDown(event: PointerEvent) {
    ...
    const intersects = raycaster.intersectObjects(getSelectableEditorObjectsForViewport(), true)
    const hit = intersects.find((intersection) => isNodeSelectable(getNodeIdForObject(intersection.object)))
  ...
  async function handleTransformDraggingChanged(event: { value: boolean }) {
    ...
    if (event.value) {
      ...
      for (const nodeId of dragNodeIds) {
        ...
        const parentInverse = object.parent ? object.parent.matrixWorld.clone().invert() : new THREE.Matrix4().identity()
  ...
  function prepareModalSelectionPivot() {
    ...
    if (selectedObjects.length === 1 && transformSpace === 'local') {
      selectionPivot.quaternion.copy(selectedObjects[0].getWorldQuaternion(new THREE.Quaternion()))
    } else {
  ...
  function getWorldUnitsPerPixel() {
    ...
    const rect = renderer.domElement.getBoundingClientRect()
    const distance = camera.position.distanceTo(selectionPivot.getWorldPosition(new THREE.Vector3()))
  ...
  function getSnappedTranslationPosition(position: THREE.Vector3) {
    ...
    if (transformSpace === 'local' && getTransformableSelectedNodeIds().length === 1) {
      const localDelta = translationDelta.applyQuaternion(initialPivotQuaternion.clone().invert())
      localDelta.set(
      ...
      return initialPivotPosition.clone().add(localDelta.applyQuaternion(initialPivotQuaternion))
    }
  ...
  function restoreInitialTransforms() {
    ...
    for (const nodeId of getTransformableSelectedNodeIds()) {
      ...
      const localMatrix = new THREE.Matrix4().multiplyMatrices(initial.parentInverse, initial.worldMatrix)
      const nextPosition = new THREE.Vector3()
      const nextQuaternion = new THREE.Quaternion()
      const nextScale = new THREE.Vector3()
  ...
  function applyModalTransform(pointerDelta: THREE.Vector2, axis: EditorTransformAxis) {
    if (!modalSession) return
    ...
    if (modalSession.mode === 'translate') {
      ...
      if (axis === 'all') {
        ...
        const translation = cameraRight.multiplyScalar(pointerDelta.x * unitsPerPixel)
          .add(cameraUp.multiplyScalar(-pointerDelta.y * unitsPerPixel))
        selectionPivot.position.copy(getSnappedTranslationPosition(initialPivotPosition.clone().add(translation)))
      } else {
        ...
        selectionPivot.position.copy(
          getSnappedTranslationPosition(initialPivotPosition.clone().add(axisDirection.multiplyScalar(projectedDelta * unitsPerPixel)))
        )
      }
    }
    ...
    if (modalSession.mode === 'rotate') {
      if (axis === 'all') {
        const cameraForward = camera.getWorldDirection(new THREE.Vector3())
        const cameraRight = new THREE.Vector3().crossVectors(cameraForward, camera.up).normalize()
        const yaw = new THREE.Quaternion().setFromAxisAngle(camera.up.clone().normalize(), getSnappedRotationAngle(pointerDelta.x * 0.01))
        const pitch = new THREE.Quaternion().setFromAxisAngle(cameraRight, getSnappedRotationAngle(pointerDelta.y * 0.01))
        selectionPivot.quaternion.copy(initialPivotQuaternion).premultiply(yaw).premultiply(pitch)
      } else {
        const axisDirection = getWorldAxisVector(axis)
        const screenAxis = getScreenAxisDirection(axis)
        ...
        selectionPivot.quaternion.copy(initialPivotQuaternion).premultiply(rotation)
      }
    }

    if (modalSession.mode === 'scale') {
      const factor = getSnappedScaleFactor(Math.max(0.05, Math.exp((pointerDelta.x - pointerDelta.y) * 0.01)))
      if (axis === 'all') {
  ...
  function updateModalTransform(clientX: number, clientY: number) {
    if (!modalSession) return
    const pointerDelta = new THREE.Vector2(clientX - modalPointerStart.x, clientY - modalPointerStart.y)
  ...
  function handleKeyDown(event: KeyboardEvent) {
    ...
    if (mod && event.key.toLowerCase() === 'd') {
      event.preventDefault()
      const transformableSelectedNodeIds = getTransformableSelectedNodeIds()
      if (transformableSelectedNodeIds.length > 0) duplicateNodes(transformableSelectedNodeIds)
      return
    }
    ...
    if (!mod && event.shiftKey && event.key.toLowerCase() === 'z') {
      event.preventDefault()
      setEditorViewportLightingMode(viewportLightingMode === 'workbench' ? 'authored' : 'workbench')
      return
    }
    ...
    switch (event.key.toLowerCase()) {
      ...
      case 'r':
        event.preventDefault()
        if (interactionMode === 'objects' && getTransformableSelectedNodeIds().length > 0) beginModalTransform('rotate')
        else setTransformMode('rotate')
        break
      case 's':
        event.preventDefault()
        if (interactionMode === 'objects' && getTransformableSelectedNodeIds().length > 0) beginModalTransform('scale')
