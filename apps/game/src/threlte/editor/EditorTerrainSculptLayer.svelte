<script lang="ts">
  ...
  function rebuildPreviewGeometry(heightData: Float32Array | null = terrainState?.heightData ?? null) {
    if (!previewGeometry || !terrainState?.manager || !heightData || terrainState.resolution === 0) return
    ...
    previewGeometry.setAttribute('position', new THREE.BufferAttribute(positions, 3))
  ...
  function updateBrushVisual() {
    ...
    brushRing.position.set(intersectionPoint.x, intersectionPoint.y + 0.15, intersectionPoint.z)
    brushFill.position.set(intersectionPoint.x, intersectionPoint.y + 0.08, intersectionPoint.z)
    brushRing.scale.setScalar(radius)
    brushFill.scale.setScalar(radius)

    const color = editorState?.terrainBrushMode === 'smooth'
      ? '#7cf8d2'
  ...
  function updatePointer(event: PointerEvent | MouseEvent) {
    ...
    const intersections = previewMesh ? raycaster.intersectObject(previewMesh, false) : []
    ...
    if (terrainState?.manager) {
      intersectionPoint.y = terrainState.manager.getHeightAt(intersectionPoint.x, intersectionPoint.z)
    }
  ...
  function applyBrushAt(worldX: number, worldZ: number, lower = false) {
    ...
    for (let z = Math.max(0, gridCenter.indexZ - radiusZ); z <= Math.min(resolution - 1, gridCenter.indexZ + radiusZ); z += 1) {
      for (let x = Math.max(0, gridCenter.indexX - radiusX); x <= Math.min(resolution - 1, gridCenter.indexX + radiusX); x += 1) {
        ...
        if (mode === 'raise') {
          ...
          nextHeight = THREE.MathUtils.lerp(currentHeight, flattenTargetHeight, Math.min(1, strength * influence))
        } else {
          let neighborSum = 0
          let neighborCount = 0
          for (let sampleZ = Math.max(0, z - 1); sampleZ <= Math.min(resolution - 1, z + 1); sampleZ += 1) {
            for (let sampleX = Math.max(0, x - 1); sampleX <= Math.min(resolution - 1, x + 1); sampleX += 1) {
              neighborSum += sourceData[sampleZ * resolution + sampleX]
              neighborCount += 1
            }
          }
          const average = neighborCount > 0 ? neighborSum / neighborCount : currentHeight
          nextHeight = THREE.MathUtils.lerp(currentHeight, average, Math.min(1, strength * influence * 0.6))
        }

        sculptHeightData[index] = THREE.MathUtils.clamp(nextHeight, minHeight, maxHeight)
      }
    }
  ...
  function handlePointerDown(event: PointerEvent) {
    ...
    flattenTargetHeight = terrainState.manager?.getHeightAt(intersectionPoint.x, intersectionPoint.z) ?? 0
  ...
  onMount(() => {
    ...
    rendererRef?.domElement.addEventListener('pointerdown', handlePointerDown, { capture: true })
    rendererRef?.domElement.addEventListener('pointermove', handlePointerMove, { capture: true })
  ...
  onDestroy(() => {
    unsubEditor()
    unsubTerrain()
    rendererRef?.domElement.removeEventListener('pointerdown', handlePointerDown, { capture: true })
    rendererRef?.domElement.removeEventListener('pointermove', handlePointerMove, { capture: true })
