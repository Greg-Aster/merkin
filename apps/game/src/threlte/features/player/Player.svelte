...
import {
  type PlayerState,
  multiplayerStore,
} from '../multiplayer/stores/multiplayerStore'
import {
  frameTimeStore,
...
function handleTouchMove(event: TouchEvent) {
  ...
  const touch = Array.from(event.changedTouches).find(
    ({ identifier }) => identifier === surfaceTouchId,
  )
  if (!touch) return
  event.preventDefault()
  ...
  if (
    !surfaceTouchForwardActive &&
    movedDistance > SURFACE_HOLD_MOVE_THRESHOLD &&
    surfaceMoveHoldTimeout !== null
  ) {
    window.clearTimeout(surfaceMoveHoldTimeout)
    surfaceMoveHoldTimeout = null
...
function getGamepadInput() {
  ...
  gamepadState.moveX = applyStickDeadzone(
    pad.axes[0] ?? 0,
    GAMEPAD_MOVE_DEADZONE,
  )
  gamepadState.moveZ = applyStickDeadzone(
    pad.axes[1] ?? 0,
    GAMEPAD_MOVE_DEADZONE,
  )
  gamepadState.lookX = applyStickDeadzone(
    pad.axes[2] ?? 0,
    GAMEPAD_LOOK_DEADZONE,
  )
  gamepadState.lookY = applyStickDeadzone(
    pad.axes[3] ?? 0,
    GAMEPAD_LOOK_DEADZONE,
  )
  gamepadState.jump = Boolean(pad.buttons[0]?.pressed)
  gamepadState.sprint = Boolean(
    pad.buttons[5]?.pressed || pad.buttons[7]?.pressed,
  )
  gamepadState.pulse = Boolean(
    pad.buttons[1]?.pressed || pad.buttons[6]?.pressed,
  )
  return gamepadState
}
...
function mixShockwaveColor(from: THREE.Color, to: THREE.Color, amount: number) {
  return `#${shockwaveColorScratch
    .copy(from)
    .lerp(to, Math.min(1, Math.max(0, amount)))
    .getHexString()}`
}
...
function stopChargeAudio() {
  ...
  try {
    ...
    localMasterGain?.gain.setValueAtTime(
      Math.max(localMasterGain.gain.value, 0.0001),
      now,
    )
...
async function playReleaseAudio(strength: number) {
  ...
  primaryOscillator.type =
    strength >= LIGHT_BURST_THRESHOLD ? 'sawtooth' : 'triangle'
  harmonicOscillator.type = 'sine'
  ...
  envelope.gain.exponentialRampToValueAtTime(
    Math.max(peakVolume, 0.0002),
    now + 0.018,
  )
  envelope.gain.exponentialRampToValueAtTime(0.0001, now + duration)

  primaryOscillator.frequency.setValueAtTime(startFrequency, now)
  primaryOscillator.frequency.exponentialRampToValueAtTime(
    endFrequency,
    now + duration,
  )
...
function updateChargeAudio(chargeAmount: number, flashAmount: number) {
  ...
  chargeHarmonicOscillator.frequency.setTargetAtTime(
    baseFrequency * 1.5,
    now,
    0.05,
  )
  chargeFilterNode.frequency.setTargetAtTime(880 + brightness * 2600, now, 0.05)
  chargeMasterGain.gain.setTargetAtTime(
    resolvePlayerSfxVolume(0.06 + brightness * 0.14),
    now,
    0.045,
  )
}

function spawnLightShockwave(
  origin: [number, number, number],
  strength: number,
) {
  const maxScale =
    LIGHT_SHOCKWAVE_RADIUS_BASE + strength * LIGHT_SHOCKWAVE_RADIUS_BONUS
  shockwaves = [
    ...shockwaves,
    {
      id: ++shockwaveId,
...
async function ensureMultiplayerService(): Promise<void> {
  ...
  if (!multiplayerServicePromise) {
    multiplayerServicePromise = import(
      '../multiplayer/services/MultiplayerService'
    ).then(module => {
...
useTask(delta => {
  ...
  const qualityForcesFallback =
    !qualitySettings.enableDynamicLighting || qualitySettings.canvasScale < 0.75
  const reportedFrameMs = $frameTimeStore
  const overloadDetected =
    smoothedFrameMs >= SHOCKWAVE_FALLBACK_TRIGGER_MS ||
    reportedFrameMs >= SHOCKWAVE_FALLBACK_TRIGGER_MS

  if (qualityForcesFallback || overloadDetected) {
    shockwaveFallbackHoldMs = SHOCKWAVE_FALLBACK_HOLD_MS
  } else if (
    shockwaveFallbackHoldMs > 0 &&
    smoothedFrameMs <= SHOCKWAVE_FALLBACK_RECOVER_MS &&
    reportedFrameMs <= SHOCKWAVE_FALLBACK_TRIGGER_MS
  ) {
    shockwaveFallbackHoldMs = Math.max(
      0,
      shockwaveFallbackHoldMs - delta * 1000,
    )
  }

  shockwaveContourEnabled =
    !qualityForcesFallback && shockwaveFallbackHoldMs <= 0

  if ($isSoundEnabled && lightChargeInputActive && !chargeAudioActive) {
    void beginChargeAudio()
  } else if (
    (!$isSoundEnabled || !lightChargeInputActive) &&
    chargeAudioActive
  ) {
    stopChargeAudio()
  }
  ...
  if (lightChargeInputActive) {
    lightChargeAmount = Math.min(
      1,
      lightChargeAmount + delta / LIGHT_CHARGE_MAX_TIME,
    )
  ...
  if (!lightChargeInputActive && wasLightCharging) {
    ...
    lightFlashAmount = Math.max(
      lightFlashAmount,
      Math.min(1, LIGHT_FLASH_TAP_BOOST + releaseCharge * 0.45),
    )
    ...
    shockwaves = shockwaves
      .map(shockwave => {
        ...
        return {
          ...
          electricColor: mixShockwaveColor(
          ...
          lightColor: mixShockwaveColor(
            ...
            Math.min(1, nextProgress * 0.95 + shockwave.strength * 0.15),
          ),
        }
      })
  ...
  if (Math.abs(gamepadInput.moveX) > Math.abs(input.x))
    input.x = gamepadInput.moveX
  if (Math.abs(gamepadInput.moveZ) > Math.abs(input.z))
  ...
  tempDesiredTranslation
    .set(tempHorizontalVelocity.x, playerVelocity.y, tempHorizontalVelocity.z)
...
onMount(() => {
  ...
  characterController = rapier.world.createCharacterController(
