...
import { type SpatialEntity, SpatialGrid } from './SpatialGrid'
...
export class FireflyLightingSystem {
  ...
  private ecsWorld: ECSWorldManager
  ...
  constructor(
    ...
    if (isDev) {
      console.log(
        '✨ FireflyLightingSystem: Initialized with config:',
        this.config,
      )
    }
  }
  ...
  private performLightingUpdate(deltaTime: number, camera: THREE.Camera): void {
    ...
    const lightsToActivate = this.selectFirefliesForLighting(
      visibleFireflies,
      deltaTime,
    )
    ...
    if (import.meta.env.DEV && this.frameCounter % 300 === 0) {
   ...
   */
  private selectFirefliesForLighting(
    visibleFireflies: SpatialEntity[],
    deltaTime: number,
  ): Map<string, SpatialEntity> {
    ...
    for (const firefly of visibleFireflies) {
      ...
      const sinValue =
        (Math.sin(time * this.config.twinkleSpeed + phase) + 1) / 2
  ...
  private updateLightStates(
    targetLights: Map<string, SpatialEntity>,
    deltaTime: number,
  ): void {
    ...
    for (const [entityIdStr, firefly] of targetLights) {
      ...
      if (!lightState) {
        ...
        lightState.targetIntensity =
          firefly.data.intensity * firefly.data.twinkleFade
    ...
    for (const [ownerId, lightState] of this.activeLights) {
      ...
      const fadeProgress = Math.min(
        lightState.fadeTimer / this.config.fadeSpeed,
        1.0,
      )
  ...
  updateConfig(newConfig: Partial<FireflyLightConfig>): void {
    ...
    if (isDev) {
      console.log(
        '✨ FireflyLightingSystem: Configuration updated:',
        this.config,
      )
    }
  }
  ...
  getStats(): typeof this.stats & {
    spatialGridStats: ReturnType<SpatialGrid['getStats']>
  } {
    return {
      ...this.stats,
  ...
  private logPerformanceStats(): void {
    ...
    if (isDev) {
      ...
      console.log(
        `  Lights: ${this.stats.lightsActive}/${this.config.maxLights} active`,
      )
      console.log(
        `  Light Pool: ${lightPoolStats.active}/${lightPoolStats.total} used`,
      )
      console.log(`  Update Time: ${this.stats.updateTimeMs.toFixed(2)}ms`)
      console.log(
        `  Entities Processed: ${this.stats.entitiesProcessed} (culled from ${spatialStats.totalEntities})`,
      )
      console.log(
        `  Spatial Grid: ${spatialStats.activeCells} cells, ${spatialStats.averageEntitiesPerCell.toFixed(1)} avg entities/cell`,
      )
    }
  }
