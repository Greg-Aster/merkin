...
export class ConversationManager {
  ...
  async startConversation(
    ...
    if (this.config.enableLogging && isDev) {
      console.log(
        `🗣️ Started conversation with ${personality.name} (${sessionId})`,
      )
    }
  ...
  async endConversation(sessionId: string): Promise<void> {
    ...
    if (this.config.enableLogging && isDev) {
      console.log(
        `🏁 Ended conversation ${sessionId} (${Math.round(duration / 1000)}s)`,
      )
    }
  }
    ...
    try {
      ...
      this.emitEvent({
        ...
        data: {
          ...
          context: `Processing message in session ${sessionId}`,
        },
      })
  ...
  private async getAIResponse(
    ...
    const enhancedMemoryManager = new (
      await import('./MemoryManagerAgent')
    ).MemoryManagerAgent({
    ...
    const enhancedPersona = enhancedMemoryManager.buildEnhancedPersona(
      personality,
      memoryBundle,
    )
    ...
    const tokenSavings =
      memoryBundle.totalTokensEstimate > 0
        ? ((oldSystemTokenEstimate - memoryBundle.totalTokensEstimate) /
            oldSystemTokenEstimate) *
          100
        : 0
    ...
    try {
      ...
      if (this.config.enableLogging && isDev) {
        console.log('🤖 AI Service Request:', {
          ...
          worldKnowledgeCount:
            requestData.persona?.worldKnowledge?.contextualMemories?.length ||
            0,
          memoryTitles:
            requestData.persona?.worldKnowledge?.contextualMemories?.map(
              (m: any) => m.topic,
            ) || [],
        })
      }
  ...
  private async generateGreeting(
    ...
    if (
      'openingStatement' in personality.knowledge &&
      typeof (personality.knowledge as any).openingStatement === 'string'
    ) {
  ...
  private cleanupExpiredSessions(): void {
    ...
    if (expiredSessions.length > 0 && this.config.enableLogging && isDev) {
      console.log(
        `🧹 Cleaned up ${expiredSessions.length} expired conversations`,
      )
    }
  }
