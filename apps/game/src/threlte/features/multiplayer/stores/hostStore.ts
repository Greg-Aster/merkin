...
const createHostStore = () => {
  ...
  return {
    ...
    setRoomRegistered: (isRoomRegistered: boolean) =>
      update(s => ({ ...s, isRoomRegistered })),
    ...
    updatePlayers: (players: { peerId: string }[]) =>
      update(s => ({ ...s, players })),
  }
}
...
export async function registerRoom(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Network request failed. Status: ${response.status}, Body: ${errorBody}`,
      )
    }
