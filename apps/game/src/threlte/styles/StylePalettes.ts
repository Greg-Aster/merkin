 ...
 */
export const staticStylePalettes: Record<
  Exclude<StylePreset, 'site' | 'surreal-site'>,
  ColorPalette
> = {
...
function resolveCssColorValue(
  ...
  try {
    ...
    console.warn(
      'Failed to resolve CSS color value for palette:',
      cssValue,
      error,
    )
    return fallback.clone()
...
function getRootHue() {
  ...
  const parsedHue = Number.parseFloat(
    rootStyles.getPropertyValue('--hue').trim(),
  )
...
export function createSitePalette(): ColorPalette {
  ...
  return {
    sky: resolveCssColorValue(
      `oklch(${backgroundL} 0.018 ${hue})`,
      fallback.sky,
    ),
    skyGradient: resolveCssColorValue(
      `oklch(${darkMode ? 0.28 : 0.9} 0.035 ${hue})`,
      fallback.skyGradient,
    ),
    water: resolveCssColorValue(
      `oklch(${darkMode ? 0.42 : 0.68} 0.08 ${hue + 30})`,
      fallback.water,
    ),
    grass: resolveCssColorValue(
      `oklch(${darkMode ? 0.48 : 0.78} 0.07 ${hue + 55})`,
      fallback.grass,
    ),
    earth: resolveCssColorValue(
      `oklch(${surfaceL} 0.02 ${hue})`,
      fallback.earth,
    ),
    trees: resolveCssColorValue(
      `oklch(${darkMode ? 0.38 : 0.58} 0.06 ${hue + 38})`,
      fallback.trees,
    ),
    flowers: resolveCssColorValue(
      `oklch(${accentL} 0.12 ${hue + 12})`,
      fallback.flowers,
    ),
    fireflies: [
      resolveCssColorValue(
        `oklch(${darkMode ? 0.82 : 0.76} 0.16 ${hue + 58})`,
        fallback.fireflies[0],
      ),
      resolveCssColorValue(
        `oklch(${darkMode ? 0.8 : 0.74} 0.14 ${hue + 35})`,
        fallback.fireflies[1],
      ),
      resolveCssColorValue(
        `oklch(${darkMode ? 0.78 : 0.72} 0.13 ${hue + 20})`,
        fallback.fireflies[2] ?? fallback.fireflies[0],
      ),
    ],
    ambient: resolveCssColorValue(
      `oklch(${darkMode ? 0.34 : 0.86} 0.03 ${hue})`,
      fallback.ambient,
    ),
    sun: resolveCssColorValue(
      `oklch(${darkMode ? 0.88 : 0.8} 0.1 ${hue + 30})`,
      fallback.sun,
    ),
    shadow: resolveCssColorValue(
      `oklch(${darkMode ? 0.12 : 0.42} 0.03 ${hue})`,
      fallback.shadow,
    ),
    fog: resolveCssColorValue(
      `oklch(${darkMode ? 0.28 : 0.9} 0.018 ${hue})`,
      fallback.fog,
    ),
    outline: resolveCssColorValue(
      `oklch(${darkMode ? 0.06 : 0.28} 0.015 ${hue})`,
      fallback.outline,
    ),
  }
}
...
export function createSurrealSitePalette(): ColorPalette {
  ...
  return {
    ...
    skyGradient: blendColors(
      sitePalette.skyGradient,
      new THREE.Color('#22125f'),
      0.72,
    ),
...
export function findClosestPaletteColor(
  ...
  if (targetColor instanceof THREE.Color) {
    ...
    try {
      ...
      console.warn(
        'Could not convert target color to THREE.Color:',
        targetColor,
        error,
      )
      return palette.earth
