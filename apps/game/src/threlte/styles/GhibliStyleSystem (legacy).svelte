<!--
  Ghibli Style System for MEGAMEAL
  
  Implements global visual styling with toon shading, outlines, and 
  curated color palettes inspired by Studio Ghibli aesthetics.
-->
<script lang="ts">
...
export let ambientIntensity = 100.4
...
const stylePalettes = {
  ghibli: {
    ...
    fireflies: [
      ...
      new THREE.Color('#FFAC33'),
      new THREE.Color('#FF6347'),
    ], // Keep warm for contrast
...
function convertSceneToToonShading() {
  ...
  console.log(
    `🎨 Converted ${processedMaterials.size} materials to toon shading`,
  )
}
...
function createToonMaterial(
  ...
  if (
    objectName.toLowerCase().includes('tree') ||
    objectName.toLowerCase().includes('birch') ||
    objectName.toLowerCase().includes('maple')
  ) {
    ...
    objectName.toLowerCase().includes('water') ||
    objectName.toLowerCase().includes('ocean')
  ) {
...
<!-- Enhanced lighting for stylized look (optional) -->
{#if isInitialized && enableStyleLighting}
  
  <!-- Ambient light with palette color -->
  <T.AmbientLight 
...
<!-- Scene fog for depth and atmosphere (always enabled) -->
{#if isInitialized}
  <T.Fog
