...
const scaledSpawnPoint: [number, number, number] = [
  $editorSceneStore?.settings?.level?.spawn?.position?.[0] ??
    playerSpawnPoint[0],
  FLOOR_Y +
    (($editorSceneStore?.settings?.level?.spawn?.position?.[1] ??
      playerSpawnPoint[1]) -
      -0.08) *
      SCALE,
  $editorSceneStore?.settings?.level?.spawn?.position?.[2] ??
    playerSpawnPoint[2],
]
...
function createFloatingAnomalies(): FloatingAnomaly[] {
  const anchors: Array<{
    ...
    {
      id: 'anomaly-left',
      position: createVector3(-2.5, FLOOR_Y + 1.8, 2.0),
      rotation: createVector3(0.5, 0.3, 0),
      drift: createVector3(0.12, 0.08, 0.04),
      color: COL_ACCENT,
      accent: '#8fd8ff',
      geometry: 'octahedron',
      args: [1.0, 0],
    ...
    {
      id: 'anomaly-right',
      position: createVector3(2.8, FLOOR_Y + 1.5, 1.5),
      rotation: createVector3(0.7, 0.5, 0.3),
      drift: createVector3(0.2, 0.1, 0.16),
      color: '#00ff88',
      accent: '#c8fff4',
    ...
    {
      id: 'anomaly-back-left',
      position: createVector3(-4.2, FLOOR_Y + 2.3, 2.8),
      rotation: createVector3(0.4, 0.6, 0.2),
      drift: createVector3(0.08, 0.4, 0.06),
      color: COL_TRIM,
      accent: '#d9f4ff',
    ...
    {
      id: 'anomaly-back-right',
      position: createVector3(4.6, FLOOR_Y + 1.6, 2.1),
      rotation: createVector3(0.3, 0.5, 0.4),
      drift: createVector3(0.1, 0.12, 0.1),
      color: '#ff0088',
      accent: '#ffd0f0',
  ...
  return anchors.map((anchor, index) => {
    ...
    const satellites = Array.from(
      ...
      (_, satelliteIndex) => {
        ...
        return {
          ...
          scale: createVector3(
            randomRange(random, 0.7, 1.3),
            randomRange(random, 0.7, 1.4),
            randomRange(random, 0.7, 1.3),
          ),
          color: anchor.accent,
    ...
    const shards = Array.from({ length: 6 }, (_, shardIndex) => {
      const angle =
        (shardIndex / 6) * Math.PI * 2 + randomRange(random, -0.2, 0.2)
      return {
        id: `${anchor.id}-shard-${shardIndex}`,
        geometry: 'box' as const,
        args: [0.07, randomRange(random, 0.45, 0.9), 0.09],
        position: createVector3(
          Math.cos(angle) * randomRange(random, 0.6, 1.0),
          randomRange(random, -0.25, 0.55),
          Math.sin(angle) * randomRange(random, 0.6, 1.0),
        ),
        rotation: createVector3(
    ...
    const rings = Array.from({ length: 2 }, (_, ringIndex) => ({
      ...
      rotation: createVector3(
        Math.PI / 2 + ringIndex * 0.35,
        ringIndex * 0.2,
        ringIndex === 0 ? 0 : Math.PI / 3,
      ),
      scale: createVector3(1, 1, 1),
      color: anchor.accent,
      emissive: anchor.accent,
  ...
  return items.map((item, index) => {
      ...
      (_, satelliteIndex) => {
        ...
        return {
          ...
          args:
            satelliteIndex % 2 === 0
              ? [
                  randomRange(random, 0.16, 0.42),
                  randomRange(random, 0.08, 0.18),
                  randomRange(random, 0.16, 0.42),
                ]
              : [randomRange(random, 0.12, 0.28)],
          position: createVector3(
            Math.cos(angle) * randomRange(random, 0.35, 0.85),
            randomRange(random, 0.1, 0.55),
            Math.sin(angle) * randomRange(random, 0.35, 0.85),
          ),
          rotation: createVector3(
            randomRange(random, 0, Math.PI),
            randomRange(random, 0, Math.PI),
    ...
    const shards = Array.from({ length: 3 + (index % 4) }, (_, shardIndex) => {
      ...
      return {
        ...
        position: createVector3(
          Math.cos(angle) * randomRange(random, 0.22, 0.6),
          randomRange(random, -0.1, 0.42),
          Math.sin(angle) * randomRange(random, 0.22, 0.6),
        ),
        rotation: createVector3(
      ...
      halo:
        index % 2 === 0
          ? {
              ...
              rotation: createVector3(
                Math.PI / 2 + randomRange(random, -0.5, 0.5),
                randomRange(random, 0, Math.PI),
                0,
              ),
              scale: createVector3(1, 1, 1),
...
const strangePlantOffsets: Array<[number, number]> = [
  [-3.0, 4.0],
  [3.0, 4.0],
  [-3.0, 2.0],
  [3.0, 2.0],
...
const textureBundlesToDispose = [interiorPanelTextures, junkTextures]
...
$: activeSkyboxPreset =
  SKYBOX_PRESETS[
    sciFiLevelSettings.skyboxPreset as keyof typeof SKYBOX_PRESETS
...
$: sciFiKeyLightIntensity =
  sciFiLevelSettings.lighting?.keyLightIntensity ?? 0.55
$: sciFiFillLightIntensity =
  sciFiLevelSettings.lighting?.fillLightIntensity ?? 0.2
$: sciFiStarMapEnabled = sciFiLevelSettings.features?.starMap ?? true

$: screenColor =
  displayPhase < 0.33 ? '#0066cc' : displayPhase < 0.66 ? '#00ccff' : '#6600ff'
...
const storyPoints: StoryPoint[] = [
  {
    ...
    text: 'The pillar hums first, like a machine trying to remember the language of prayer.',
  },
  {
    name: 'bench-note',
    position: createVector3(0, COURTYARD_Y + 0.7, COURTYARD_Z - 2.0),
    text: 'The bench offers a pause before the open sky, its carved warning almost tender: look up before you go farther.',
  },
  {
    name: 'fountain-inscription',
    position: createVector3(0, COURTYARD_Y + 0.8, COURTYARD_Z),
    text: 'At the courtyard heart, the fountain mirrors the constellations, as if the room is learning to become an observatory.',
  },
  {
    name: 'strange-plant',
    position: createVector3(3.0, COURTYARD_Y + 2.2, COURTYARD_Z + 4.0),
    text: 'The spiral plants lean toward one particular star cluster, suggesting the garden knows the route better than you do.',
  },
  {
    name: 'junk-memory',
    position: createVector3(-3.6, FLOOR_Y + 1.8, WASTELAND_Z + 12.4),
    text: 'Deep in the wasteland, the debris resolves into intent: this ruin was not abandoned, it was aimed at the heavens and left mid-sentence.',
  },
]
...
function handleInteractiveHover(markerId: string, hovered: boolean) {
  hoveredInteractiveId = hovered
    ? markerId
    : hoveredInteractiveId === markerId
      ? null
      : hoveredInteractiveId
}
...
function getMarkerSpriteIntensity(marker: InteractiveMarker) {
  return hoveredInteractiveId === marker.id
    ? marker.intensity * 1.25
    : marker.intensity
}
