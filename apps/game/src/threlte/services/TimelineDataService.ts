...
export class TimelineDataService {
  ...
  async loadStarEvents(
    ...
    try {
      ...
      if (options.startYear)
        params.set('startYear', options.startYear.toString())
      ...
      if (options.includeDrafts !== undefined)
        params.set('includeDrafts', options.includeDrafts.toString())
      ...
      console.log(
        `✅ TimelineDataService: Processed ${this.cachedEvents.length} star events`,
      )
      return this.filterEvents(this.cachedEvents, options)
    } catch (error) {
      console.error(
        '❌ TimelineDataService: Failed to load timeline events:',
        error,
      )
      return []
    }
   ...
   * Transform a timeline event into star data suitable for 3D display
   */
  private transformTimelineEventToStarData(
    event: TimelineEvent,
    index: number,
  ): StarEventData {
  ...
  private calculateStarPosition(
    ...
    const config =
      CONSTELLATION_CONFIG[era as keyof typeof CONSTELLATION_CONFIG]
    ...
    const elevationDeg =
      config.center[1] + (Math.random() - 0.5) * config.spread * 0.5
   ...
   */
  private checkIfEventIsLevel(event: TimelineEvent): {
    isLevel: boolean
    levelId: string | null
  } {
    ...
    const levelPatterns = [
      ...
      {
        pattern: /library|infinite.*library/i,
        levelId: 'infinite-library-level',
      },
   ...
   */
  async getEventsByEra(
    options: any = {},
  ): Promise<Record<string, StarEventData[]>> {
