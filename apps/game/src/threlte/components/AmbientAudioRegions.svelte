...
const resolvedHowlerModule =
  howlerInteropCandidates.find(
    (candidate: any) => candidate?.Howl || candidate?.Howler,
  ) ?? null
...
function syncSoundsWithRegions(nextRegions: AmbientAudioRegion[]) {
  ...
  nextRegions.forEach(region => {
    ...
    const normalizedTrack = Array.isArray(currentTrack)
      ? currentTrack[0]
      : currentTrack
...
function computeInfluence(region: AmbientAudioRegion) {
  ...
  const [sx, sy, sz] = region.scale.map(value => Math.abs(value) / 2) as [
    number,
    number,
    number,
  ]
...
useTask(() => {
  ...
  regions.forEach(region => {
    ...
    const targetVolume =
      $masterVolumeSetting * $ambienceVolumeSetting * region.volume * influence
