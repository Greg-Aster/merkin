...
export let colors = [
  0x87ceeb, 0x98fb98, 0xffffe0, 0xdda0dd, 0xf0e68c, 0xffa07a, 0x20b2aa,
  0x9370db,
]
...
export let getHeightAt: ((x: number, z: number) => number) | undefined =
  undefined
...
function refreshFireflySettings() {
  currentFireflySettings =
    optimizationManager.getComponentSettings('hybrid-firefly-component') || {}
}
...
$: optimizedCount =
  currentFireflySettings.maxParticleCount ||
  (optimizedMaxLights === 0 ? Math.min(count, 30) : count)
$: optimizedPointSize = currentFireflySettings.particleSize || pointSize
$: optimizedEnableGlow =
  currentFireflySettings.enableGlow !== undefined
    ? currentFireflySettings.enableGlow
    : true
$: optimizedEnableInteractions =
  currentFireflySettings.enableInteractions !== undefined
    ? currentFireflySettings.enableInteractions
    : true
...
class HybridFireflyComponent extends BaseLevelComponent {
  ...
  protected async onInitialize(): Promise<void> {
    if (isDev) {
      console.log(
        '✅ HybridFirefly: Initializing with modern ECS architecture...',
      )
    }
    if (!ecsWorld) {
    ...
    if (isDev) {
      console.log(
        `✅ HybridFirefly: Created ${optimizedCount} ECS entities with StarSprite visuals`,
      )
    }
  }
  ...
  private createECSFireflies(): void {
    ...
    if (!getHeightAt) {
      if (isDev) {
        console.warn(
          '🧚 Warning: No terrain height function available - fireflies may spawn below ground',
        )
      }
    } else {
      ...
      if (isDev) {
        console.log(
          `🧚 Firefly spawn: Terrain function working, center height = ${testHeight.toFixed(2)}`,
        )
      }
    }
    ...
    for (let i = 0; i < optimizedCount; i++) {
      ...
      const minHeight = Math.max(
        groundHeight + heightRange.min,
        groundHeight + safetyMargin,
      )
      ...
      if (import.meta.env.DEV && i < 2 && y < groundHeight) {
        console.warn(
          `⚠️ Firefly ${i} spawned below ground: y=${y.toFixed(2)}, ground=${groundHeight.toFixed(2)}`,
        )
      }
  ...
  public async setupVisualFireflies(): Promise<void> {
    ...
    if (enableAIConversations) {
      const conversationalCount = Math.floor(
        optimizedCount * conversationChance,
      )
      ...
      try {
        ...
        for (let i = 0; i < conversationalCount; i++) {
          const characterId =
            availableConversationalCharacterIds[
              i % availableConversationalCharacterIds.length
            ]
        ...
        if (isDev)
          console.log(
            `🧠 Auto-discovered and assigned ${characterAssignments.length} characters to conversational fireflies`,
          )
      } catch (error) {
        ...
        if (isDev)
          console.warn(
            '🔄 Disabling AI conversations due to character discovery failure',
          )
      }
    }
    ...
    visualFireflies = fireflyEntities.map((eid, i) => {
      ...
      const hasCharacterAssignment =
        enableAIConversations && i < characterAssignments.length
      const characterId = hasCharacterAssignment
        ? characterAssignments[i]
        : null

      // Use character data if available, otherwise use basic system
      const species = hasCharacterAssignment
        ? 'Lost Soul'
        : 'Common Eastern Firefly'
      const name =
        hasCharacterAssignment && characterId
          ? characterId
              .split('-')
      ...
      const basicPersonality =
        basicPersonalities[
          Math.floor(Math.random() * basicPersonalities.length)
        ]

      return {
        id: eid,
        position: [Position.x[eid], Position.y[eid], Position.z[eid]] as [
          number,
          number,
          number,
        ],
    ...
    if (isDev)
      console.log(
        `✨ Created ${visualFireflies.length} fireflies (${characterAssignments.length} with character personalities)`,
      )
  }
  ...
  private updateVisualFireflies(time: number): void {
    ...
    for (let i = 0; i < entities.length && i < visualFireflies.length; i++) {
      ...
      visual.intensity =
        Math.min(baseIntensity * intensityNorm, 1.0) * fadeProgress
      ...
      const twinkle = optimizedEnableGlow
        ? 0.85 + twinkle1 + twinkle2 + twinkle3
        : 1.0
      ...
      const finalOpacity = Math.min(
        1,
        visual.intensity * twinkle * (1.0 + hoverGlow),
      )
...
onMount(async () => {
  ...
  if (isDev) {
    console.log(
      '🧚‍♀️ HybridFirefly: Registered component-specific optimization settings',
    )
  }
...
async function handleFireflyClick(data: any) {
  ...
  if (
    enableAIConversations &&
    firefly.isConversational &&
    firefly.characterId
  ) {
    // It's a conversational firefly. Use the ConversationManager.
    if (isDev) {
      console.log(
        `🤖 Starting AI conversation with ${firefly.name} using character: ${firefly.characterId}`,
      )
    }
    await handleConversationalFirefly(firefly)
    gameActions.recordInteraction(
      'firefly_ai_conversation',
      firefly.id || 'unknown',
    )
  } else {
...
async function handleConversationalFirefly(firefly: any) {
  try {
    const { characterRegistry, conversationActions } =
      await loadConversationFeature()
    ...
    if (character) {
      ...
      const npcPersonality =
        characterRegistry.convertToNPCPersonality(character)
      ...
      if (isDev)
        console.log(
          '✅ Started AI conversation via store system with converted personality',
        )
    } else {
...
async function handleBasicFirefly(firefly: any) {
  ...
  const randomSpecies =
    poeticSpecies[Math.floor(Math.random() * poeticSpecies.length)]
...
function handleSpriteReady(
  sprite: Sprite,
  firefly: FireflyVisual,
  index: number,
) {
  ...
  if (interactionSystem && firefly.isClickable) {
    ...
    interactionSystem.registerInteractiveObject({
      ...
      handlers: {
        ...
        onHover: (data: any, hovered: boolean) =>
          handleFireflyHover(data, hovered),
      },
    })
  } else {
    if (isDev && firefly.isClickable) {
      console.warn(
        `⚠️ Failed to register firefly: interactionSystem=${!!interactionSystem}, clickable=${firefly.isClickable}`,
      )
    }
  }
}
...
export function setEmotionalState(
  wonder: number,
  melancholy: number,
  hope: number,
  discovery: number,
