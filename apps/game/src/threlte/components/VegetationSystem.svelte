...
onMount(async () => {
  ...
  try {
    ...
    if (isDev) {
      console.log(
        `✅ Vegetation system initialized with ${vegetationEntities.length} plants`,
      )
    }
  } catch (error) {
    ...
    ecsWorld.addSystem(
      defineSystem(world => {
        ...
        for (let i = 0; i < entities.length; i++) {
          ...
          const windEffect =
            Math.sin(windPhase) * VegetationWind.swayAmplitude[eid]
...
async function populateVegetation() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (distance < 50) {
      ...
      variant = Math.floor(
        Math.random() * (vegType === 1 ? 6 : vegType === 2 ? 3 : 7),
      )
    } else {
...
onDestroy(() => {
  ...
  if (ecsWorld && vegetationEntities.length > 0) {
    if (isDev) {
      console.log(
        `🧹 Cleaning up ${vegetationEntities.length} vegetation entities`,
      )
    }
    vegetationEntities = []
  }
