...
export const fireflyQuery = defineQuery([
  FireflyTag,
  Position,
  Velocity,
  LightEmitter,
...
export const emotionalResponderQuery = defineQuery([
  EmotionalResponder,
  LightEmitter,
])
export const playerQuery = defineQuery([PlayerTag, Position])
export const floatingEntitiesQuery = defineQuery([
  Position,
  Velocity,
  FloatingBehavior,
])
...
export class ECSWorldManager {
    ...
    this.addSystem(
      defineSystem(world => {
        ...
        for (let i = 0; i < entities.length; i++) {
          ...
          const wanderX =
            Math.cos(time * 0.1 + eid) * behavior.wanderRadius[eid] * 0.1
          const wanderZ =
            Math.sin(time * 0.1 + eid * 1.3) * behavior.wanderRadius[eid] * 0.1
    ...
    this.addSystem(
      defineSystem(world => {
        ...
        for (let i = 0; i < entities.length; i++) {
          ...
          const wonderInfluence =
            emotionalState.wonder * responder.wonderFactor[eid] * 0.01
          const melancholyInfluence =
            emotionalState.melancholy * responder.melancholyFactor[eid] * 0.01
          const hopeInfluence =
            emotionalState.hope * responder.hopeFactor[eid] * 0.01
          const discoveryInfluence =
            emotionalState.discovery * responder.discoveryFactor[eid] * 0.01
          ...
          const originalIntensity =
            this.originalIntensities.get(eid) || LightEmitter.intensity[eid]
          const emotionalMultiplier =
            1.0 + wonderInfluence + hopeInfluence - melancholyInfluence * 0.5
  ...
  updateEmotionalState(
    ...
    if (wonder !== undefined)
      EmotionalState.wonder[this.emotionalStateEntity] = wonder
    if (melancholy !== undefined)
      EmotionalState.melancholy[this.emotionalStateEntity] = melancholy
    if (hope !== undefined)
      EmotionalState.hope[this.emotionalStateEntity] = hope
