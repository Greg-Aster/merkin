...
export const enableToonShadingStore = writable<boolean>(false)
...
export class StyleManager {
  ...
  getColor(colorName: keyof ColorPalette): THREE.Color {
    ...
    return color instanceof THREE.Color
      ? color.clone()
      : this.currentPalette.earth.clone()
  }
  ...
  styleObject(
    ...
    object.traverse(child => {
      if (this.isMesh(child) && child.material) {
        const mesh = child
        ...
        mesh.material = Array.isArray(mesh.material)
          ? mesh.material.map(material =>
              this.convertToToonMaterial(material, child.name),
            )
          : this.convertToToonMaterial(mesh.material, child.name)
...
export class StyleSystemComponent implements LevelComponent {
  ...
  handleMessage(message: SystemMessage): void {
    if (
      message.type === MessageType.COMPONENT_READY &&
      message.data.componentType === ComponentType.ENVIRONMENT
    ) {
