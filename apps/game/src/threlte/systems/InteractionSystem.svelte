...
function updateLightBursts(timestamp: number) {
  const deltaSeconds = Math.min(
    0.05,
    Math.max(0.001, (timestamp - lastLightBurstFrameTime) / 1000),
  )
  ...
  activeLightBursts = activeLightBursts.filter(burst => {
    ...
    burst.radius = Math.min(
      burst.maxRadius,
      burst.radius + burst.speed * deltaSeconds,
    )

    for (const object of interactiveObjects) {
      if (!object.handlers.onLightBurst || burst.hitObjectIds.has(object.id))
        continue
...
function applyHoveredObject(
  ...
  if (newHoveredId !== hoveredObjectId) {
    ...
    if (hoveredObjectId) {
      const prevObject = interactiveObjects.find(
        obj => obj.id === hoveredObjectId,
      )
...
export function getScreenPosition(worldPosition: THREE.Vector3): {
  x: number
  y: number
} {
