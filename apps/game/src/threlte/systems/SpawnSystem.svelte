...
export function requestSpawn(request: Omit<SpawnRequest, 'id'>) {
  ...
  const existingRequest = spawnQueue.find(
    r =>
      r.entityType === request.entityType && r.component === request.component,
  )

  if (existingRequest) {
    if (isDev)
      console.log(
        `⚠️ SpawnSystem: Duplicate spawn request ignored for ${request.entityType}`,
      )
    return false
  }
  ...
  if (isDev)
    console.log(
      `📝 SpawnSystem: Queued ${request.entityType} spawn at [${request.position.join(', ')}]`,
    )
...
async function processSpawnQueue() {
  ...
  if (isDev)
    console.log(
      `🔄 SpawnSystem: Processing ${spawnQueue.length} spawn requests`,
    )
  ...
  try {
    ...
    if (success) {
      ...
      if (isDev)
        console.log(
          `✅ SpawnSystem: Successfully spawned ${request.entityType} at [${request.position.join(', ')}]`,
        )
...
async function executeSpawn(request: SpawnRequest): Promise<boolean> {
  switch (request.entityType) {
    ...
    default:
      console.error(
        `❌ SpawnSystem: Unknown entity type: ${request.entityType}`,
      )
      return false
  }
}
...
async function spawnPlayer(request: SpawnRequest): Promise<boolean> {
  ...
  if (!isValidPlayerComponent(playerComponent)) {
    console.warn(
      'SpawnSystem: Player spawn requested before player component was ready.',
    )
    return false
  }
...
export function clearSpawnQueue() {
  if (isDev)
    console.log(
      `🧹 SpawnSystem: Clearing spawn queue (${spawnQueue.length} pending)`,
    )
  spawnQueue = []
