...
export type AssistantBannerData = {}
...
export const bannerConfig: BannerConfig = {
  // Default banner type for main pages
  defaultBannerType: 'timeline', // Can be 'standard', 'video', 'image', 'timeline', 'assistant', or 'none',
  ...
  layout: {
    height: {
      desktop: '80vh', // 80% of viewport height on desktop
      mobile: '80vh', // 80% of viewport height on mobile
    },
    overlap: {
 ...
 */
export function getResponsiveBannerDimensions(isMobile = false): {
  height: string
  overlap: string
} {
  return {
 ...
 */
export function getNavbarHeight(isMobile = false): string {
  return isMobile
    ? bannerConfig.navbar.height.mobile
    : bannerConfig.navbar.height.desktop
}
...
export function isNoneBannerData(data: any): data is NoneBannerData {
  ...
  ) // A simple check if it's truly an empty object
}
