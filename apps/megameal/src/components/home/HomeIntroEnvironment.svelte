...
// biome-ignore lint/style/useConst: Astro passes this as a component prop.
...
function handleWheel(event: WheelEvent) {
	if (!shell || !isInsideShell(event.clientX, event.clientY)) return;

	const viewportHeight = Math.max(window.innerHeight, 1);
	input.wheel += event.deltaY / viewportHeight;
}
