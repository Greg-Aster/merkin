  ...
  archivePanels.forEach(panel => {
    ...
    Object.entries(postsByYear).forEach(([year, posts]) => {
      ...
      const yearElement = Array.from(
        panel.querySelectorAll('.text-2xl.font-bold'),
      ...
      if (yearElement) {
        ...
        createNewYearSection(panel, Number.parseInt(year), posts)
      }
    })
...
function enhanceExistingYearSection(yearElement, posts) {
  ...
  if (postCountEl) {
    ...
    if (countMatch) {
      const currentCount = Number.parseInt(countMatch[1])
...
function sortYearSections(panel) {
  ...
  yearSections.sort((a, b) => {
    const yearA = Number.parseInt(
      a.querySelector('.text-2xl.font-bold')?.textContent || '0',
    )
    const yearB = Number.parseInt(
      b.querySelector('.text-2xl.font-bold')?.textContent || '0',
    )
    return yearB - yearA
  })
...
function handleFriendContentToggle(e) {
  ...
  if (friendContentEnabled !== enabled) {
    ...
    if (enabled && isAuthenticated) {
      ...
      document
        .querySelectorAll('[data-friend-content="true"]')
