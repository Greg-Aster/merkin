...
export class SharedChatManager {
  ...
  private speechFadeCleanupListener:
  ...
  constructor(config: MascotChatConfig) {
    ...
    this.greetingMessages =
      config.customGreetingMessages || cuppyGreetingMessages || []
    this.randomDialogues =
      config.customRandomDialogues || cuppyRandomDialogues || []
  ...
  public initialize(): boolean {
    ...
    this.speechBubble = this.config.speechBubbleId
      ? document.getElementById(this.config.speechBubbleId)
      : null
    this.speechText = this.config.speechTextId
      ? document.getElementById(this.config.speechTextId)
      : null
    this.chatInputElement = document.getElementById(
      this.config.chatInputId,
    ) as HTMLInputElement
    ...
    if (
      ...
      !this.mascotVisualArea ||
      !this.chatInputElement ||
      !this.chatSendButton
    ) {
    ...
    if (!this.speechBubble || !this.speechText) {
      console.warn(
        'SharedChatManager: Speech bubble elements not found - speech functionality will be disabled',
      )
    }
  ...
  private loadMascot() {
    ...
    this.mascotImageDisplay = this.mascotVisualArea.querySelector(
      'img',
    ) as HTMLImageElement
  ...
  private displayEphemeralSpeech(textChunk: string, isLastChunk = true) {
    // Mascot animation
    if (this.mascotImageDisplay && cuppyMascotData.type === 'image-set') {
      const mascotImages = cuppyMascotData.images
      if (mascotImages?.openmouth) {
        clearTimeout(this.mouthOpenTimeout)
        this.mascotImageDisplay.src = mascotImages.openmouth
        this.mouthOpenTimeout = setTimeout(() => {
          if (mascotImages.standard && this.mascotImageDisplay) {
            this.mascotImageDisplay.src = mascotImages.standard
          }
        }, 700)
      }
    ...
    if (this.speechFadeCleanupListener) {
      this.speechBubble.removeEventListener(
        'animationend',
        this.speechFadeCleanupListener,
      )
      this.speechFadeCleanupListener = undefined
    }

    // Reset bubble state
    this.speechBubble.className = this.speechBubble.className
      .replace(/appearing|fading/g, '')
      .trim()
    ...
    const onAppearAnimationEnd = (event: AnimationEvent) => {
      ...
      currentLingerDuration = Math.max(
        3000,
        Math.min(currentLingerDuration, 4500),
      )
      ...
      this.speechLingerTimeout = setTimeout(() => {
        ...
        this.speechBubble.addEventListener(
          'animationend',
          this.speechFadeCleanupListener as EventListener,
          { once: true },
        )
  ...
  private async handleSendMessage() {
    ...
    try {
      const pageContextForPayload =
        this.pageContext && this.pageContext.trim() !== ''
          ? this.pageContext
      ...
      const response = await fetch(
        'https://my-mascot-worker-service.greggles.workers.dev',
        {
          method: 'POST',
  ...
  private triggerProactiveDialogue() {
    ...
    if (this.randomDialogues && this.randomDialogues.length > 0) {
      const randomDialogue =
        this.randomDialogues[
          Math.floor(Math.random() * this.randomDialogues.length)
        ]
  ...
  private chunkText(fullText: string, wordsPerChunk = 15): string[] {
