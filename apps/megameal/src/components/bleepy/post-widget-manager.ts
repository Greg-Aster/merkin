...
import {
  type ChatMessage,
  type MascotChatConfig,
  SharedChatManager,
} from './shared-chat-manager.ts'
...
export class PostWidgetManager {
  ...
  public initialize(): boolean {
    ...
    this.speechBubbleTemplate = document.getElementById(
  ...
  private overrideHandleSendMessage() {
    const originalHandleSendMessage = (
      this.chatManager as any
    ).handleSendMessage.bind(this.chatManager)
  ...
  private renderUserMessage(content: string) {
    ...
    messageDiv.className =
      'message user mb-3 p-3 rounded-lg bg-blue-100 dark:bg-blue-900/30 ml-8 mr-2'

    const roleSpan = document.createElement('span')
    roleSpan.className =
      'font-semibold text-xs uppercase tracking-wide mb-1 block text-blue-700 dark:text-blue-300'
  ...
  private renderAssistantMessage(content: string) {
    ...
    const bubbleClone = this.speechBubbleTemplate.content.cloneNode(
      true,
    ) as DocumentFragment
    const avatarImg = bubbleClone.querySelector(
      '.speech-bubble-avatar',
    ) as HTMLImageElement
    const contentDiv = bubbleClone.querySelector('.speech-bubble-content')

    const mainMascotImg = document.getElementById(
      'widget-mascot-image-display',
    ) as HTMLImageElement
