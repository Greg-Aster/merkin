...
$: eventPositions = events.map((event, index) => {
  ...
  const eraConfig =
    event.era && eraConfigs[event.era] ? eraConfigs[event.era] : null
...
function handleTouchEnd(e: TouchEvent) {
  ...
  if (wasTap && touchTarget) {
    ...
    if (eventNode) {
      ...
      touchTarget === timelineContainer ||
      touchTarget.classList.contains('timeline-container')
    ) {
...
onMount(() => {
  if (timelineContainer) {
    ...
    timelineContainer.addEventListener('touchmove', handleTouchMove, {
      passive: false,
    })
...
export function panToYear(targetYear: number) {
  ...
  if (!containerWidth || !containerHeight || !timespan || timespan === 0) {
    console.warn('Cannot pan: missing dimensions', {
      containerWidth,
      containerHeight,
      timespan,
    })
...
export function setPosition(
  ...
  if (
    ...
    Number.isNaN(newOffsetX) ||
    Number.isNaN(newOffsetY)
  ) {
...
export function navigateToEraRange(eraStartYear: number, eraEndYear: number) {
  ...
  if (eraConfig?.zoomLevel !== undefined) {
    ...
    console.log(
      `Calculated zoom level: ${targetZoom} (ratio: ${zoomRatio.toFixed(2)})`,
    )
  }
  ...
  setTimeout(() => {
    ...
    console.log(
      `Timeline positioned at year ${yearToCenter} with zoom ${targetZoom}`,
    )
  }, 300)
}
</script>
