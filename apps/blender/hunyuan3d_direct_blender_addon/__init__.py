...
bl_info = {
    "name": "Merkin Local Hunyuan3D",
    "author": "Tencent Hunyuan3D, packaged for Merkin",
    "version": (1, 0, 2),
    "blender": (5, 0, 0),
    "location": "View3D > Sidebar > Merkin Hunyuan3D",
    "description": "Generate textured 3D models from images through the local Merkin Hunyuan3D server",
    "category": "3D View",
}
import base64
import os
...
class Hunyuan3DProperties(bpy.types.PropertyGroup):
    prompt: StringProperty(
        name="Text Prompt",
        description="Reserved for future text-to-image support. The local direct server currently requires an image.",
        default=""
    )
    api_url: StringProperty(
        name="API URL",
        description="URL of the local Hunyuan3D image-to-3D API service",
...
class Hunyuan3DOperator(bpy.types.Operator):
    ...
    bl_description = "Generate a 3D model from an image through the local direct Hunyuan3D server"
    ...
    def invoke(self, context, event):
        ...
        if self.image_path == "":
            self.report({'WARNING'}, "Select an image first. The local direct Hunyuan3D server does not support text-only generation.")
            return {'FINISHED'}
        ...
        if self.selected_mesh and self.texture:
            ...
            props.status_message = f"Generating {mesh_type} from Image...\n" \
                                   "This may take several minutes depending \n on your GPU power."
...
class Hunyuan3DPanel(bpy.types.Panel):
    ...
    bl_region_type = 'UI'
    bl_category = 'Merkin Hunyuan3D'
    bl_label = 'Local Hunyuan3D Generator'
    ...
    def draw(self, context):
        ...
        layout.label(text="Local image-to-3D. Select an image before generating.")
